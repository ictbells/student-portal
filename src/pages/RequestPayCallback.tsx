...
export default function RequestPayCallback() {
  ...
  const [message, setMessage] = useState(
    token && reference ? 'Confirming your payment…' : 'Missing payment reference or request token.',
  );
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get(`/api/public-pay/${encodeURIComponent(token)}/verify/${encodeURIComponent(reference)}${
      ...
      .then(({ data }) => {
        ...
        setMessage('Payment confirmed. Staff will email you when your request is ready.');
      ...
      <header className="border-b border-slate-200/80 bg-white">
        <div className="mx-auto flex max-w-lg items-center gap-3 px-4 py-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-semibold text-slate-900 truncate">Request & pay</p>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {status === 'verifying' ? (
            ...
            <div className="mt-6 space-y-2">
              <Link
                to={token ? `/request-pay?token=${encodeURIComponent(token)}` : '/request-pay'}
