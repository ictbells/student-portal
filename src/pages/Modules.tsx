...
export function Academic() {
  ...
  const register = async (courseOfferingId: number) => {
    try {
      ...
      toast.error(e.response?.data?.message || 'Could not register this course.');
    }
  };
  ...
  const drop = async (enrollmentId: number) => {
    try {
      ...
      toast.error(e.response?.data?.message || 'Could not drop this course.');
    }
  };
  ...
  const requestExtension = async () => {
    try {
      ...
      toast.error(e.response?.data?.message || 'Could not request an extension.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <PageHeader title="Academic" description="Course registrations and academic standing." />
      </div>
      {clearance && (
        <Card>
      ...
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {reg?.term?.name ? `${reg.term.session_label || ''} ${reg.term.name}`.trim() : 'Current semester'}
            </p>
          </div>
          <StatusBadge status={windowStatus === 'Open' ? 'paid' : windowStatus === 'Late' ? 'partial' : 'unpaid'} />
        </div>
        {loadingReg && !reg && <Spinner label="Loading registration…" />}
        {reg && (
          <div className="space-y-4 text-sm">
            {windowStatus === 'Open' && <Alert tone="info">Registration is open. Add or drop courses until the normal window closes.</Alert>}
            ...
            {windowStatus === 'Late' && paidExtension && (
              <Alert tone="success">Your extension is paid. You can add or drop courses until {extension?.expires_at ? new Date(extension.expires_at).toLocaleString() : 'the late window closes'}.</Alert>
            )}
            {windowStatus === 'Closed' && (
            ...
            {!reg.tuition_ok && (
              <Alert tone="warning">Pay at least 25% of current-session tuition before registering courses. <Link to="/invoices" className="text-sky-700 underline">Open invoices</Link></Alert>
            )}

            <p>
              Window: <span className="font-medium">{windowStatus}</span>
              {' · '}Roster: <span className="font-medium">{String(reg.roster_status || 'not started').replaceAll('_', ' ')}</span>
              {' · '}Tuition: <span className="font-medium">{Math.round(reg.tuition_percent || 0)}%</span>
            </p>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {['overall', ...COURSE_BUCKETS.map((b) => b.value)].map((bucket) => {
                const limit = reg.limits?.[bucket] || {};
                const used = reg.units?.[bucket] ?? 0;
                return (
                  <div key={bucket} className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">{bucket}</p>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {group.rows.map((row: any) => (
                      <li key={row.id} className="py-2 flex justify-between gap-3 items-center">
                        ...
                        {canMutate && !row.is_carry_over && (
                          <Button type="button" className="bg-white border border-slate-200" onClick={() => drop(row.id)}>Drop</Button>
                        )}
                      </li>
                    ))}
                  </ul>
            ...
            {canMutate && (
              <div>
                ...
                {availableGroups.every((group) => group.rows.length === 0) && <p className="text-slate-500">No offerings are available for you this semester.</p>}
                {availableGroups.map((group) => group.rows.length > 0 && (
                  <div key={group.value} className="mb-3">
                    ...
                    <ul className="divide-y divide-slate-100">
                      {group.rows.map((row: any) => (
                        <li key={row.id} className="py-2 flex justify-between gap-3 items-center">
                          <span>{row.course?.code} {row.course?.title} ({row.course?.units} units) · {row.seats_left} seats</span>
                          <Button type="button" onClick={() => register(row.id)}>Register</Button>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="space-y-2 rounded-xl border border-slate-200 p-3">
                ...
                {(!extension || ['rejected', 'expired', 'cancelled'].includes(extension.status)) && (
                  <>
                    ...
                    <Button type="button" onClick={requestExtension} disabled={!reason}>Submit request</Button>
                  </>
                )}
              </div>
            )}
